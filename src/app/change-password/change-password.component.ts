        ...
        this.httpWebService.updatePasswordRequest(this.userid, this.data, this.password)
          .subscribe((resp: { status: string; }) => {
            if (resp.status === '200') {
              ...
              this.httpWebService.createNotification(this.userid, 'Password Change Successful', this.errorDetl + ' If you did not request this change immediately contact us at support@rrs.in');
            } else {
              this.errorDetl = 'Sorry! Password updation failed!';
              // tslint:disable-next-line:max-line-length
              this.httpWebService.createNotification(this.userid, 'Password Change Failed', this.errorDetl + ' If you did not request this change immediately contact us at support@rrs.in');
            }
          });
      }
  }
